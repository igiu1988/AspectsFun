...
static NSMethodSignature *aspect_blockMethodSignature(id block, NSError **error) {
    ...
    // 如果有，把reserved 和 size 跳过去了 wangyang
	void *desc = layout->descriptor;
	desc += 2 * sizeof(unsigned long int);

    // 如果有，把 Copy、Dispose 跳过去了 wangyang
    ...
    /* wangyang
     const char *mysignature = layout->descriptor->signature;
     这个地方不能直接这样获取这个 signature。因为 descriptor 这个结构体是由编译器生成的，编译器只会生成必要的数据，其中的cop与dispose就可能是不存在的，这样的话在执行第一句话“AspectBlockRef layout = (__bridge void *)block;”时，真正的signature就对应到了copy上，所以这里只能使用移动指针的方式来取得正确的signature
     */
...
static BOOL aspect_isCompatibleBlockSignature(NSMethodSignature *blockSignature, id object, SEL selector, NSError **error) {
    ...
    // blockSignature:
    // methodSignature: self, selector, parameter
